...
void ParentProcess::parent_process_handler() {
    ...
    if (pid == 0) {
        run_process(CHILD_NAME, file_name.c_str(), mmf_file_name, mutex_name, is_empty_sem_name, is_full_sem_name);
        ...
        int64_t* mmf_file_ptr = static_cast<int64_t*>(create_shm_ptr(mmf_file_desc));
